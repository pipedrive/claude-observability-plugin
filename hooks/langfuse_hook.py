...
def _is_turn_start(msg: Dict[str, Any]) -> bool:
    ...
    if not isinstance(msg, dict) or msg.get("isMeta"):
        return False
    if get_user_or_assistant_role_from_row(msg) != "user":
        return False
...
def emit_subagent(langfuse: Langfuse, sub_path: Path, parent_otel_span: Any,
    ...
    for st in sub_turns:
        st_user_text, _ = truncate_text(extract_text(get_content_from_row(st.user_msg)))
        _emit_assistant_steps(
            ...
            start_ts=parse_timestamp(st.user_msg),
